...
pub fn get_phone_and_bert(gpts: &GPTSovits, text: &str) -> anyhow::Result<(Tensor, Tensor)> {
    ...
    phone_builder.push_punctuation(&gpts.symbols, "\u{7a7a}");
        ...
        match self.sentence.back_mut() {
            ...
            _ => {
                if let Some(phones) = arpabet.get_polyphone_str(&word) {
                    let mut phones_ids = vec![];
                    for ph in &phones {
                        phones_ids.push(get_phone_symbol(symbols, ph));
                    }
                    self.sentence.push_back(Sentence::En(EnSentence {
                        phones_ids,
                        phones,
